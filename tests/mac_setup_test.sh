...
if [ "${1:-}" = "install" ] && [ "${2:-}" = "-g" ]; then
  guard_write_path "$HOME/.npm-global/bin"
  ...
  for package in "${@:3}"; do
    ...
    guard_write_path "$HOME/.npm-global/bin/$bin"
    cat > "$HOME/.npm-global/bin/$bin" <<'BIN'
#!/bin/bash
set -euo pipefail
# shellcheck source=/dev/null
. "${SANDBOX_GUARD:?}" || exit 1
...
EOF
  ...
  assert_contains "$invocations" "npm install -g @openai/codex" \
    "$name: Codex CLI install path exercised" && pass "$name: Codex CLI install path exercised"
  assert_contains "$invocations" "npm install -g @earendil-works/pi-coding-agent" \
    "$name: Pi install path exercised" && pass "$name: Pi install path exercised"
  assert_contains "$invocations" "npm install -g skills" \
